...
local txn_in="$1"                     # Save input file into txn_in
local txn_out="$2"                    # Store cleaned file into txn_out

# Our "friends" at Dermatology Associates decided to put a comma in their description, sigh.
# I'm sure other "friends" will do the same.
sed -e 's/DERMATOLOGY ASSOCIATES, 910-763-1555/DERMATOLOGY ASSOCIATES/' "$txn_hold" >"$txn_working"
...
$2 ~ /STARBUCKS STORE/ { $2 = "Starbucks"; $3 = "Food & Dining:Coffee Shops" }
...
#
#---------------------------------------------------------------------------------------------------
$2 ~ /DOR TAX PAYMENTS/ { $2 = "NC Dept of Revenue"; $3 = "Taxes:Local Taxes" }  # Issue #37
...
awk --file="$awk_program" "$txn_working"
